...
def _infer_nhl_season_year(game_dt: Optional[datetime]) -> int:
    """
    Store NHL season as the *ending year* (e.g., Oct 2024 -> 2025 season).
    """
    if not game_dt:
...
async def fetch_espn_nhl_scoreboard(*, date_yyyymmdd: str) -> Dict[str, Any]:
    params = {"dates": date_yyyymmdd}

    async with httpx.AsyncClient(timeout=25.0, headers={"User-Agent": "SportLytics/1.0"}) as client:
        r = await client.get(settings.ESPN_NHL_SCOREBOARD_URL, params=params)
        r.raise_for_status()
        return r.json()


def parse_espn_nhl_scoreboard(payload: Dict[str, Any]) -> List[NHLGameRow]:
    ...
    for ev in events:
        ...
        season_year = (payload.get("season") or {}).get("year") or _infer_nhl_season_year(date)
        ...
        source_url = (ev.get("links") or [{}])[0].get("href") or "https://www.espn.com/nhl/"
