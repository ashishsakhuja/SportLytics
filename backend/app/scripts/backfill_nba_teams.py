...
def _iter_team_blobs(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    # ESPN teams response is usually nested under sports -> leagues -> teams
    ...
    for s in sports:
        for lg in (s.get("leagues") or []):
            for t in (lg.get("teams") or []):
                ...
                if team:
                    out.append(team)
    # Fallback if ESPN ever returns top-level teams
...
def _parse_team(team: Dict[str, Any]) -> Optional[Tuple[str, str, Optional[str]]]:
    ...
    # ESPN: location + name are common, but sometimes displayName is best
...
async def fetch_espn_nba_teams() -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=25.0, headers={"User-Agent": "SportLytics/1.0"}) as client:
        r = await client.get(settings.ESPN_NBA_TEAMS_URL)
        r.raise_for_status()
        return r.json()


async def main_async() -> None:
    payload = await fetch_espn_nba_teams()
    ...
    try:
        ...
        for t in teams:
            ...
            existing = db.query(Team).filter(Team.sport == "nba", Team.team_code == code).one_or_none()
            if existing:
                existing.name = full_name
                existing.city = city
            else:
                db.add(Team(sport="nba", team_code=code, name=full_name, city=city, meta=None))
            upserted += 1

        db.commit()
        print(f"[NBA] backfill teams upserted={upserted}")
    finally:
        db.close()
