...
async def fetch_espn_nhl_teams() -> Dict[str, Any]:
    async with httpx.AsyncClient(timeout=25.0, headers={"User-Agent": "SportLytics/1.0"}) as client:
        r = await client.get(settings.ESPN_NHL_TEAMS_URL)
        r.raise_for_status()
        return r.json()


async def main_async() -> None:
    payload = await fetch_espn_nhl_teams()
    ...
    try:
        ...
        for t in teams:
            ...
            existing = db.query(Team).filter(Team.sport == "nhl", Team.team_code == code).one_or_none()
            if existing:
                existing.name = full_name
                existing.city = city
            else:
                db.add(Team(sport="nhl", team_code=code, name=full_name, city=city, meta=None))
            upserted += 1

        db.commit()
        print(f"[NHL] backfill teams upserted={upserted}")
    finally:
        db.close()
