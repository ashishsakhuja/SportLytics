...
def _filter_real_teams(sport: str, rows: list[Team]) -> list[Team]:
    allowed = _allowed_team_codes_for_sport(sport)
    if not allowed:
        return rows
    filtered = [t for t in rows if (t.team_code or "").upper().strip() in allowed]
...
def list_teams(
    ...
    rows = _filter_real_teams(sport, rows)
    return {
        "sport": sport,
        "count": len(rows),
        "teams": [
